...
const MyProfile = () => {
    ...
    return (
        <section className=''>
            <h1 className='text-center text-white text-3xl py-3 font-bold border-b-2 border-b-gray-300'>My Profile</h1>
            <div className='flex md:flex-row flex-col md:p-10 p-4 md:space-x-10'>
                <div>
                    <div className="avatar online">
                        <div className="w-24 md:w-32 rounded-full">
                            <img src={user?.photoURL  ? user?.photoURL :  `https://i.ibb.co/K035fHn/149071.png` } alt='Profile' />
                        </div>
                    </div>
                    {
                        toogleProfileEdit ?
                        ...
                        <form onSubmit={handleSubmit(handeProfileUpdate)}>
                            <div className='space-y-2'>
                                <div className='space-y-1'>
                                    <label className='text-sm text-white font-bold'>Enter Your Full Name</label>
                                    <input
                                        {...register('name', {
                                            required: {
                                                value: true,
                                        ...
                                        type="text" defaultValue={userData?.name && userData.name} placeholder={userData?.name ? userData?.name : "Sabbir Ahmed"} className="input input-bordered w-full max-w-xs" />
                                ...
                                <div className='space-y-1'>
                                    <label className='text-sm text-white font-bold'>Enter Your Email</label>
                                    <input value={user?.email} type="text" placeholder="example@mail.com" className="input input-bordered w-full max-w-xs" />
                                </div>
                                <div className='space-y-1'>
                                    <label className='text-sm text-white font-bold'>Enter Your Phone</label>
                                    <input
                                        {...register('phone', {
                                            required: {
                                                value: true,
                                        ...
                                        type="text" defaultValue={userData?.phone && userData.phone} placeholder={userData?.phone ? userData.phone : "+8801xxxxxxxxx"} className="input input-bordered w-full max-w-xs" />
