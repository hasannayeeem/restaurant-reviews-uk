...
const Address = () => {
    ...
    const handleUpdateAddress = data => {
        ...
        fetch(`http://localhost:5000/api/v1/user?email=${user?.email}`, {
            ...
            .then(data => {
                console.log(data)
                toast.success('successfully updated your addresses')
            })
        reset()
    }
    ...
    return (
        <section className=''>
            <h1 className='text-center text-white text-3xl py-3 font-bold border-b-2 border-b-gray-300'>My Address</h1>

            {
                toogleAddressEdit ?
                    <form onSubmit={handleSubmit(handleUpdateAddress)}>
                        <div className='grid text-white grid-cols-1 lg:grid-cols-3 gap-5 p-4'>
                            <div>
                    ...
                    :
                    <div className='flex md:flex-row text-white flex-col md:p-10 p-4 '>
                        <div className='md:pr-28'>
                            ...
                            <div className='grid lg:grid-cols-2 grid-cols-1 md:mt-5'>
                                <div>
                                    ...
                                    <p>{userData?.currentCountry ? userData.currentCountry : "Bangladesh"}</p>
                                </div>
                                <div className='lg:pl-10'>
                                    <h4 className='text-sm font-bold'>Your Town</h4>
                                    <p>{userData?.currentAddress ? userData.currentAddress : "Dhaka"}</p>
                                </div>
                                <div className='md:mt-5'>
                                    <h4 className='text-sm font-bold'>Zip Code</h4>
                                    <p>{userData?.currentZip ? userData.currentZip : "7830"}</p>
                                </div>
                            </div>
                        </div>
                        <div>
                            ...
                            <div className='grid lg:grid-cols-2 grid-cols-1 md:mt-5'>
                                <div>
                                    <h4 className='text-sm font-bold'>Your County</h4>
                                    <p>{userData?.country ? userData.country : "Bangladesh"}</p>
                                </div>
                                <div className='lg:pl-10'>
                                    <h4 className='text-sm font-bold'>Your Town</h4>
                                    <p>{userData?.address ? userData.address : "Dhaka"}</p>
                                </div>
                                <div className='md:mt-5'>
                                    <h4 className='text-sm font-bold'>Zip Code</h4>
                                    <p>{userData?.zip ? userData.zip : "7830"}</p>
                                </div>
                            </div>
                        </div>
                    </div>
...

            {
                toogleAddressEdit ?
                    ...
                    :
                    <div className='text-center'>
                        <button onClick={() => setToogleAddressEdit(!toogleAddressEdit)} className='bg-blue-600 font-semibold py-2 px-5 mb-4 text-white rounded-full'>Edit Address</button>
                    </div>
            }
        </section>
    );
